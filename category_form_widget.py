...
from categories import Cagtegories
from category_list import categories
...
class CategoryFormWidget(QWidget):
    def __init__(self, parent, text_edit):
        super(CategoryFormWidget, self).__init__(parent)
        self.all = False
    ...
    def init_UI(self):
        ...
        btn_layout.addWidget(run_btn, alignment=Qt.AlignRight)

        middle_layout.addWidget(m, alignment=Qt.AlignLeft)
    ...
    def call_run(self, c_obj):
        import chipsec_main
        custom_categories = c_obj.checked_exec()
        # for c in custom_categories:
        #     print json.dumps(
        #         categories[c['index']], indent=1
        #     )
        result = str()
        ...
        for custom_category in custom_categories:
            for module in categories[custom_category['index']]['modules']:
                command = list()
                print module
                command.append('-m')
        ...
        #         os.remove("log.txt")                    
        # else:
        #     for custom_module in  custom_categories:
        #         command = list()
